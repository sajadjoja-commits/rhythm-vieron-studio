 ...
 */
export const createLazyComponent = async (importFn: () => Promise<any>) => {
  return importFn();
};
...
export const logPerformanceMetrics = () => {
  if (typeof window !== 'undefined' && 'performance' in window) {
    ...
    if (perfAny.memory) {
      console.log(`  Memory Used: ${(perfAny.memory.usedJSHeapSize / 1048576).toFixed(2)}MB`);
      console.log(`  Memory Limit: ${(perfAny.memory.jsHeapSizeLimit / 1048576).toFixed(2)}MB`);
    }
  }
};
