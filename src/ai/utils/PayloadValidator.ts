...
export class PayloadValidator {
  ...
  public static detectMediaType(rawPayload: any): "image" | "audio" | "video" | "general" {
    ...
    // 2. Explicit flag or field presence
    ...
    if (/\.(mp4|webm|mov|avi|mkv|m4v|3gp|flv)(\?.*)?$/.test(lower)) return "video";
    if (/\.(mp3|wav|m4a|ogg|flac|aac)(\?.*)?$/.test(lower)) return "audio";

    // 4. Tool ID or taskType keywords
  ...
  public static normalize(rawPayload: any): NormalizedAIPayload {
    ...
    const detectedType = this.detectMediaType(rawPayload);
    const inputMediaType = detectedType === "general"
      ? (rawPayload.domain || rawPayload.mediaType || "image")
      : detectedType;
    ...
    const blobCandidate =
      rawPayload.file ||
      rawPayload.blob ||
      rawPayload.videoFile ||
      rawPayload.imageBlob ||
      rawPayload.audioFile ||
      undefined;

    // Strict Domain Isolation: Populate the matching media property
  ...
  public static validate(payload: any, domain: PayloadDomain = "general"): PayloadValidationResult {
    ...
    if (domain === "image") {
      if (!normalized.imageBase64OrUrl && normalized.mediaUrlOrBase64) {
        normalized.imageBase64OrUrl = normalized.mediaUrlOrBase64;
      }
      ...
      // Universal fallback for video input handoff
      if (!normalized.videoBase64OrUrl && normalized.mediaUrlOrBase64) {
        normalized.videoBase64OrUrl = normalized.mediaUrlOrBase64;
      }
      if (!normalized.videoFile && normalized.imageBlob && normalized.imageBlob.type.startsWith("video/")) {
        normalized.videoFile = normalized.imageBlob;
      }
      ...
      if (!normalized.audioBase64OrUrl && normalized.mediaUrlOrBase64) {
        normalized.audioBase64OrUrl = normalized.mediaUrlOrBase64;
      }
