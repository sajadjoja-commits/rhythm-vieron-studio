...
export interface ModelManifest {
  ...
  inputShape: number[];
  outputShape: number[];
  preferredProviders?: ("webgpu" | "wasm" | "cpu")[];
}
...
export class ONNXModelLoader {
  ...
  /**
   * Purge session from in-memory cache
   */
  public purgeSessionCache(modelId: string, version?: string): void {
    if (version) {
      this.sessionCache.delete(`${modelId}_${version}`);
    } else {
  ...
  private async createInferenceSession(
    ...
    const preferred = manifest.preferredProviders;
    const allowsWebGPU = !preferred || preferred.includes("webgpu");
    const hasWebGPU = allowsWebGPU && typeof navigator !== "undefined" && "gpu" in navigator && !!navigator.gpu;
