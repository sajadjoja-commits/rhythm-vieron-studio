...
export class ImageEnhancementPlugin extends BasePlugin {
  ...
  public async execute<TPayload = AIImagePayload, TResult = AIImageResult>(
    ...
    try {
      ...
      switch (action) {
        ...
        case "enhance":
        case "upscale":
          result = await this.runEnhance(imgPayload, options);
          break;
  ...
  private async runEnhance(
    ...
    const src = payload.imageBase64OrUrl;
    let aiRes = await imageAIEngine.enhanceImage(src, {
      denoiseIntensity: payload.denoiseIntensity ?? 0.75,
      contrastBoost: (payload as any).contrastBoost ?? 0.6,
      detailSharpen: (payload as any).detailSharpen ?? 0.5,
      signal: options?.signal,
    });

    // Optional face restoration if requested and present
    if ((payload as any).enhanceFace) {
      try {
        const faceRes = await imageAIEngine.enhanceFace(aiRes.outputDataUrl, {
          enhanceFaceLevel: (payload as any).enhanceFaceLevel ?? 0.85,
          signal: options?.signal,
        });
        aiRes = faceRes;
      } catch {
        // Continue with enhanced image if no face detected
      }
    }
  ...
  private async runCompositeEnhance(
    ...
    return {
      ...
      mimeType: payload.mimeType || "image/png",
      width: enhanceRes.width,
      height: enhanceRes.height,
      processingType: "composite-enhance",
      appliedEngine: "Unified Pipeline (Denoise -> Face Restore -> Adaptive HDR Detail Sharpen)",
      executionTimeMs: 0,
      qualityMetrics: {
        isLocalExecution: true,
      },
