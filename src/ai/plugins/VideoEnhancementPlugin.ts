...
export class VideoEnhancementPlugin extends BasePlugin {
  ...
  public async execute<TPayload = AIVideoPayload, TResult = AIVideoResult>(
    ...
    try {
      const vidPayload = payload as unknown as AIVideoPayload;

      if (!vidPayload || !vidPayload.videoBase64OrUrl) {
        return {
          success: false,
          error: this.createError("INVALID_PAYLOAD", "Video input (videoBase64OrUrl) is required"),
        };
      }
      ...
      // 1. Check Cache
      const inputHash = aiRuntime.aiManager.cache.generateHash(`plugin_video_${action}`, vidPayload);
      if (options?.enableCache !== false) {
        const cachedMatch = aiRuntime.historyManager.findMatch("enhance-media", inputHash);
  ...
  private async runVideoUpscale(payload: AIVideoPayload, isLocal: boolean): Promise<AIVideoResult> {
    ...
    const remoteRes = await aiRuntime.aiManager.enhanceMedia(payload.videoBase64OrUrl, {
