...
export async function saveImageToGallery(
  ...
  try {
    ...
    const cacheUri = writeResult.uri;

    try {
      if (VireonMedia && VireonMedia.saveImageToGallery) {
        const result = await VireonMedia.saveImageToGallery({ path: cacheUri });
        if (result && result.success) {
          return { success: true, path: cacheUri };
        }
      }
    } catch (pluginErr) {
    ...
    console.error("[NativeService] saveImageToGallery failed:", err);
    throw err;
  }
}
