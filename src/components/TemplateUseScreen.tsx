...
import { 
  ...
  Sparkles, RefreshCw, Type, Music, Lock, Check, Loader2, Volume2, VolumeX, Hash, Trash2 
} from "lucide-react";
...
export default function TemplateUseScreen({ templateId, templateObj, onBack }: Props) {
  ...
  const handleDeleteTemplate = async () => {
    ...
    try {
      ...
      if (ok) {
        toast.success(isRTL() ? "تم حذف القالب بنجاح" : "Template deleted successfully");
        setShowDeleteModal(false);
        onBack();
      } else {
        toast.error(isRTL() ? "حدث خطأ أثناء حذف القالب" : "Failed to delete template");
      }
    } catch {
      toast.error(isRTL() ? "حدث خطأ أثناء حذف القالب" : "Failed to delete template");
    } finally {
  ...
  const ratio = ASPECT_RATIOS[activeRatioIndex] || ASPECT_RATIOS[0];
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-background overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border flex-shrink-0 bg-secondary/30">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-1.5 rounded-xl bg-destructive/10 hover:bg-destructive/20 text-destructive border border-destructive/20 transition-colors"
      ...
      <AnimatePresence>
        {showDeleteModal && (
          <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" dir={isRTL() ? "rtl" : "ltr"}>
            <motion.div
