...
const EditorScreen = ({ onBack }: EditorScreenProps) => {
    ...
    for (const v of active) {
      ...
      switch (v.type) {
        ...
        case "particles":
          ...
          break;
        case "water-ripple":
          transforms.push(`translate(${Math.sin(currentTime * 6) * i * 8}px, ${Math.cos(currentTime * 8) * i * 5}px) skewX(${Math.sin(currentTime * 4) * i * 3}deg)`);
          overlayStyle = {
          ...
          filters.push(`hue-rotate(${Math.sin(currentTime * 2) * i * 15}deg) saturate(${1 + i * 0.2})`);
          break;
        case "vintage-8mm":
          filters.push(`sepia(${0.6 * i}) contrast(${1 + 0.25 * i}) brightness(${0.92 + Math.sin(currentTime * 25) * 0.04 * i})`);
          transforms.push(`translateY(${Math.sin(currentTime * 35) * i * 2}px)`);
          overlayStyle = {
          ...
          break;
        case "old-vhs-tape":
          filters.push(`contrast(${1.15 + i * 0.1}) saturate(${1.3 + i * 0.2}) drop-shadow(2px 0 0 rgba(255,0,0,0.5)) drop-shadow(-2px 0 0 rgba(0,255,255,0.5))`);
          transforms.push(`translate(${(Math.random() - 0.5) * i * 3}px, ${(Math.random() - 0.5) * i * 2}px)`);
          overlayStyle = {
            background: `repeating-linear-gradient(rgba(0,0,0,0) 0px, rgba(0,0,0,${0.2 * i}) 2px, rgba(0,0,0,0) 4px), linear-gradient(to bottom, transparent ${(currentTime * 30) % 100}%, rgba(192,38,211,0.2) ${((currentTime * 30) % 100) + 2}%, transparent ${((currentTime * 30) % 100) + 4}%)`,
            pointerEvents: "none"
          };
          break;
        case "retro-sepia-grain":
          filters.push(`sepia(${0.85 * i}) contrast(${1.1 + i * 0.2}) brightness(${0.9 + i * 0.05})`);
          overlayStyle = {
          ...
          break;
        case "crt-tv-retro":
          filters.push(`contrast(${1.2 + i * 0.1}) brightness(${1.05}) saturate(${1.2})`);
          overlayStyle = {
          ...
          transforms.push(`perspective(500px) scale(${1 - i * 0.02})`);
          break;
        default:
          break;
      }
  ...
  useEffect(() => {
    ...
    // Avoid jarring seek when smoothly playing across split clips of the same media file
    const isPlayingCurrentMedia = isPlayingRef.current && !v.paused;
    const threshold = isPlayingCurrentMedia ? 0.45 : 0.15;
