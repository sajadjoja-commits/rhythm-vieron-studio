      ...
      >
        <div className="absolute top-0 left-0 h-full" style={{
          ...
          {packed.map(({ item: f, lane }) => {
            ...
            return (
              <div key={f.id} className={`absolute rounded-lg flex items-center cursor-grab overflow-visible border shadow-md ${focused ? "border-cyan-400 ring-1 ring-cyan-400/50" : "border-cyan-500/40"}`}
                style={{ left, width: w, height: 24, top: (focused ? lane : 0) * ROW + 2, background: `linear-gradient(135deg, rgba(6,182,212,0.5), rgba(6,182,212,0.25))` }}
                onPointerDown={(e) => onDown(e, f, "move")}>
                {focused && (
                  <TimelineTrimHandle side="left" onPointerDown={(e) => onDown(e, f, "left")} />
                )}
                ...
                <span className="text-[8px] text-white/60 px-0.5">{Math.round(f.intensity * 100)}%</span>
                {focused && (
                ...
                {focused && (
                  <TimelineTrimHandle side="right" onPointerDown={(e) => onDown(e, f, "right")} />
                )}
              </div>
            );
          })}
