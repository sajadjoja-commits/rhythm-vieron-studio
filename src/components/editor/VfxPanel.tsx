...
import { X, Sparkles, Trash2, Plus, Check, Eye, EyeOff } from "lucide-react";
...
const VFX_LIB: { type: VfxType; label: string; labelEn: string; emoji: string; color: string; desc: string; descEn: string }[] = [
  { type: "water-ripple",        label: "تموج الماء",    labelEn: "Water Ripple",  emoji: "🌊", color: "#06b6d4", desc: "تموج موجات الماء الحية", descEn: "Fluid ripple wave distortion" },
  { type: "vintage-8mm",         label: "سينما 8مم قديم", labelEn: "8mm Vintage",   emoji: "🎥", color: "#d97706", desc: "شريط أفلام 8 مم الكلاسيكي القديم", descEn: "Classic 8mm cinema film style" },
  { type: "old-vhs-tape",        label: "شريط VHS قديم",  labelEn: "Old VHS Tape",  emoji: "📼", color: "#c026d3", desc: "شريط فيديو أنالوج قديم مع شريط التتبع", descEn: "Retro analog tape with tracking noise" },
  { type: "retro-sepia-grain",   label: "سيبيا ريترو",    labelEn: "Sepia Grain",   emoji: "📜", color: "#b45309", desc: "لون سيبيا دافئ مع حبيبات ريترو 1920", descEn: "1920s warm sepia film with heavy grain" },
  { type: "crt-tv-retro",        label: "تلفزيون CRT",   labelEn: "CRT Retro TV",  emoji: "📺", color: "#10b981", desc: "شاشة تلفاز أنالوج قديمة بوهج أنبوبي", descEn: "Vintage CRT tube glow & scanlines" },
  ...
  { type: "crt-scanner", label: "مسح CRT",       labelEn: "CRT Scan",     emoji: "📼", color: "#00ffcc", desc: "شاشة ريترو أنالوج", descEn: "Retro CRT phosphor mask" },
...
function renderVfxFrame(ctx: CanvasRenderingContext2D, type: VfxType, p: number, color: string) {
  ...
  switch (type) {
    ...
    case "water-ripple": {
      ctx.fillStyle = "#06b6d433"; ctx.fillRect(0, 0, W, H);
      for (let i = 0; i < 3; i++) {
        const rad = ((p * 20 + i * 8) % 22) + 2;
        ctx.strokeStyle = `rgba(6, 182, 212, ${1 - rad / 24})`;
        ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(W/2, H/2, rad, 0, Math.PI * 2); ctx.stroke();
      }
      break;
    }
    case "vintage-8mm": {
      ctx.fillStyle = "#d9770633"; ctx.fillRect(0, 0, W, H);
      ctx.fillStyle = "rgba(0,0,0,0.5)"; ctx.fillRect(0, 0, 6, H); ctx.fillRect(W - 6, 0, 6, H);
      for (let i = 0; i < 15; i++) {
    ...
    case "old-vhs-tape": {
      for (let y = 0; y < H; y += 2) { ctx.fillStyle = "rgba(0,0,0,0.3)"; ctx.fillRect(0, y, W, 1); }
      const lineY = (p * H * 1.5) % H;
      ctx.fillStyle = "rgba(192, 38, 211, 0.5)"; ctx.fillRect(0, lineY, W, 3);
      ctx.fillStyle = "#00ff00"; ctx.font = "8px monospace"; ctx.fillText("PLAY 00:00", 6, H - 6);
      break;
    }
    case "retro-sepia-grain": {
      ctx.fillStyle = "#b4530944"; ctx.fillRect(0, 0, W, H);
      for (let i = 0; i < 65; i++) {
    ...
    case "crt-tv-retro": {
      ctx.fillStyle = "#10b98122"; ctx.fillRect(0, 0, W, H);
      ...
      g.addColorStop(0, "rgba(16,185,129,0.2)"); g.addColorStop(1, "rgba(0,0,0,0.7)");
