...
const Timeline = memo(({ currentTime, onSeek, onOpenTransition, isPlaying, onUserScrub, onWidthChange, onPxPerSecChange, hidePlayhead, focused, pxPerSec: propPxPerSec, onOpenCover, onFocus }: Props) => {
  ...
  const clipElements = useMemo(() => {
    ...
    return clips.map((clip, idx) => {
      ...
      const cardWidth = dragging ? 68 : w;
      const cardLeft = dragging ? left + (w - 68) / 2 : left;
      ...
      return (
        <div
          ...
          style={{
            left: cardLeft,
            width: cardWidth,
            transform: dragging ? `translateX(${dragDx}px) translateY(-6px)` : undefined,
            zIndex: dragging ? 50 : undefined,
            opacity: dragId && !dragging ? 0.5 : 1,
            transition: dragId ? "none" : "all 150ms cubic-bezier(0.16, 1, 0.3, 1)",
          }}
          data-no-scrub
        >
          {idx > 0 && !dragging && (
          ...
          >
            {dragging && (
              <div className="absolute top-1 left-1 z-30 px-1.5 py-0.5 rounded-md bg-amber-400 text-slate-950 text-[10px] font-black shadow-md flex items-center gap-1">
                <span>#{idx + 1}</span>
              </div>
            )}
            <ClipThumbnails clip={clip} media={media} pxPerSec={pxPerSec} />
            
            {!dragging && focused !== false && selectedClipId === clip.id && (
              <TimelineTrimHandle
                side="left"
                variant="primary"
  ...
  }, [clips, getMediaById, pxPerSec, onOpenTransition, startTrim, startMove, removeClip, dragId, dragDx, selectedClipId, focused]);
