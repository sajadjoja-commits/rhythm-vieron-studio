  ...
  const startExport = async () => {
    ...
    if (!ffmpegRef.current) {
      ...
      try {
        ...
        // Attempt 1: Try local self-hosted core files (100% offline)
        let loaded = false;
        try {
          await ffmpeg.load({
          ...
          loaded = true;
          console.log("FFmpeg core loaded from local origin.");
        } catch (localErr) {
          console.warn("Local FFmpeg core load notice, trying CDN fallback:", localErr);
        }

        // Attempt 2: Try CDN if local load failed and online
        if (!loaded && navigator.onLine) {
          try {
            ...
            ffmpegRef.current = ffmpeg;
            loaded = true;
            console.log("FFmpeg core loaded from CDN.");
          } catch (cdnErr) {
            console.warn("CDN FFmpeg load notice:", cdnErr);
          }
        }

        if (!loaded) {
          console.warn("FFmpeg not available, falling back to Native Canvas MediaRecorder Exporter.");
        }
      } catch (err) {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      // Save frame as PNG to virtual filesystem if FFmpeg is active
      if (ffmpegRef.current) {
                ...
                {exporting && (
                  <div className="absolute inset-0 bg-black/80 backdrop-blur-md flex flex-col items-center justify-center p-4 text-center z-20 space-y-3 animate-in fade-in duration-200">
                    ...
                    <button
                      ...
                      className="mt-1 px-3.5 py-1.5 rounded-xl bg-destructive/20 hover:bg-destructive/30 text-destructive border border-destructive/30 font-bold text-xs flex items-center gap-1.5 transition-all active:scale-95"
                    >
                      <X className="w-3.5 h-3.5" /> {isRTL() ? "إلغاء التصدير" : "Cancel Export"}
                    </button>
                  </div>
                )}
              </div>
