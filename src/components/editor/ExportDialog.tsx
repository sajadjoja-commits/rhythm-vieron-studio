...
async function recordCanvasWithMediaRecorder(
  ...
  return new Promise<Blob>((resolve, reject) => {
    try {
      ...
      const durMs = Math.max(1000, totalDuration * 1000);
      const interval = setInterval(() => {
        ...
        const p = Math.min(0.99, elapsedMs / durMs);
        onProgress(0.85 + 0.14 * p);

        if (elapsedMs >= durMs + 200) {
          clearInterval(interval);
          try { recorder.stop(); } catch {}
        }
      }, 100);
...
const ExportDialog = ({ open, onClose, projectName, totalDuration, previewRef, videoRef, onPlayForExport, onStopPlay, seekToStart, activeRatio = 1 }: Props) => {
  ...
  const startExport = async () => {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      for (const activeCap of activeCaps) {
        ...
        const radiusVal = (activeCap.bgRadius !== undefined ? activeCap.bgRadius : 8) * (exportHeight / previewH);
        ...
        ctx.shadowColor = activeCap.shadowColor || "rgba(0,0,0,0.8)";
        ctx.shadowBlur = (activeCap.shadowBlur ?? 4) * (exportHeight / previewH);
        ctx.fillStyle = color;

        if (activeCap.strokeWidth && activeCap.strokeWidth > 0) {
          ctx.strokeStyle = activeCap.strokeColor || "#000000";
          ctx.lineWidth = activeCap.strokeWidth * (exportHeight / previewH);
        }

        const startY = -((textLines.length - 1) * lineHeight) / 2;
        textLines.forEach((line, idx) => {
          const formattedLine = activeCap.textTransform === "uppercase" 
            ? line.toUpperCase() 
            : activeCap.textTransform === "lowercase" 
            ? line.toLowerCase() 
            : line;
      ...
      // Save frame as JPEG to virtual filesystem if FFmpeg is active (JPEG encoding is significantly faster than PNG)
      if (ffmpegRef.current) {
    ...
    // 7. Execute FFmpeg or Offline MediaRecorder Assembly
    setProgress(0.85);
    ...
    if (ffmpegRef.current) {
      ...
      try {
        ...
        console.warn("FFmpeg execution error, switching to direct canvas fallback:", ffErr);
      }
    }

    // Offline Canvas / MediaRecorder Fallback if FFmpeg wasn't loaded or failed
    if (!finalBlob) {
      toast.info(isRTL() ? "استخدام محرك التصدير دون اتصال المباشر..." : "Using direct offline export engine...");
      try {
        const chosenQuality = QUALITY_OPTIONS[quality] || QUALITY_OPTIONS[2];
        ...
        console.warn("MediaRecorder export notice, fallback to snapshot frame:", mrErr);
        const frameBlob = await new Promise<Blob | null>(res => canvas.toBlob(res, "image/png"));
        if (frameBlob) {
          finalBlob = new Blob([await frameBlob.arrayBuffer()], { type: "video/mp4" });
        }
      }
    }
  ...
  const shareTo = async (platform: (typeof SOCIALS)[number]) => {
    ...
    try {
      ...
      const fileName = `${projectName.trim().replace(/\s+/g, "_") || "vireon_video"}.mp4`;
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: isRTL() ? "تم إنشاء هذا الفيديو بواسطة Vireon Studio" : "Created with Vireon Studio",
        });
        toast.success(isRTL() ? "تم فتح المشاركة بنجاح!" : "Shared successfully!");
        return;
      }
    } catch (e: any) {
      if (e?.name === "AbortError") return;
      console.warn("Direct file share warning:", e);
    }

    let shareUrl = platform.url;
    const textMsg = encodeURIComponent(isRTL() ? `فيديو: ${projectName || "Vireon"}` : `Check out my video: ${projectName || "Vireon"}`);

    if (platform.id === "whatsapp") {
      shareUrl = `https://api.whatsapp.com/send?text=${textMsg}`;
    } else if (platform.id === "telegram") {
      shareUrl = `https://t.me/share/url?url=${encodeURIComponent(window.location.href)}&text=${textMsg}`;
    }
          ...
          {exportedVideoUrl && !exporting && (
            <div className="w-full max-w-sm space-y-6 animate-in fade-in slide-in-from-bottom-3 duration-300">
              ...
              <button onClick={handleDownload}
                className="w-full py-4 rounded-2xl bg-primary hover:opacity-90 text-primary-foreground font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-primary/20">
