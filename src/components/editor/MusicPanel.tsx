...
import { Upload, Music2, Sparkles, Mic, Wand2, X, Loader2, Activity, Scissors, Plus, Check, Eye, EyeOff, Play, Pause, Trash2, Image as ImageIcon, Link, Film } from "lucide-react";
...
const MusicPanel = ({ open, onClose, currentTime }: Props) => {
  const { media, audioTracks, setClips, addFiles, addAudioTrack, updateAudioTrack, splitClipsAtBeats, audioBeats, setAudioBeats, videoMuted, setVideoMuted, videoVolume, setVideoVolume, videoAudioFx, setVideoAudioFx, totalDuration } = useMedia();
  ...
  const onCutAtBeats = async () => {
    ...
    try {
      ...
      if (targetBeats && targetBeats.length > 0) {
        toast.info(en ? "AI analyzing video scenes for faces, motion & energy..." : "الذكاء الاصطناعي يحلل المشاهد للحركة والوجوه والطاقة...");
        ...
        if (res.clips.length > 0) {
          ...
          toast.success(
            en
              ? `Smart Cut complete! ${res.clips.length} cuts synchronized across ${targetMedia.length} clips with AI scoring.`
              : `تم التقطيع الذكي! ${res.clips.length} قطع متزامن عبر ${targetMedia.length} مقاطع باستخدام الذكاء الاصطناعي.`
          );
        } else {
        ...
        {tab === "beat" && (
          <div className="space-y-3.5 bg-secondary/20 p-3 rounded-2xl border border-border/40">
            <p className="text-[11px] text-muted-foreground text-center leading-relaxed">
              {getLang() === "en"
                ? "Analyzes music beats and selects optimal video moments (faces, motion & energy) across all selected media"
                : "يحلل إيقاع الموسيقى ويختار أفضل اللحظات الحركية والوجوه عبر جميع مقاطع الميديا المختارة"}
            </p>
