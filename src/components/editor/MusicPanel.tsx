...
const MusicPanel = ({ open, onClose, currentTime }: Props) => {
  ...
  const onDetectBpm = async () => {
    ...
    try {
      ...
      // Apply density: 1 = every beat, 2 = every 2nd, 4 = every 4th (down-sampling)
      const step = beatDensity;
      const picked = info.beats.filter((_, i) => i % step === 0);
      // Convert beats from track-source-time to timeline-time
      ...
      toast.success(`BPM ≈ ${info.bpm} — ${beatsOnTimeline.length} ${en ? "cuts" : "قطع"}`);
      splitClipsAtBeats(beatsOnTimeline);
    } catch (e) {
      console.error(e);
        ...
        {tab === "beat" && (
          <div className="space-y-3.5 bg-secondary/20 p-3 rounded-2xl border border-border/40">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 p-3 rounded-xl gradient-primary text-primary-foreground text-xs font-bold disabled:opacity-40 transition-transform active:scale-[0.98] mt-1 shadow-lg"
            >
              {busy ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>{getLang() === "en" ? `Analyzing... ${beatProgress}%` : `جارٍ التحليل... ${beatProgress}%`}</span>
                </>
              ) : (
                <>
                  <Scissors className="w-4 h-4" />
                  <span>{getLang() === "en" ? "Analyze and auto-cut to the beat" : "تحليل وقص الفيديو على الإيقاع الذكي"}</span>
                </>
              )}
            </button>
          </div>
