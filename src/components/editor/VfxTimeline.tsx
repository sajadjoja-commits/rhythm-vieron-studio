...
const VfxTimeline = memo(({ currentTime, pxPerSec, containerW, isPlaying, focused, onSeek, onAddClick, onFocus }: Props) => {
  ...
  if (!vfx.length) return null;

  const onDown = (e: React.PointerEvent, v: VfxItem, mode: "move" | "left" | "right") => {
    e.stopPropagation();
    onFocus?.();
    let startXAdjusted = e.clientX;
    lastPointerXRef.current = e.clientX;
    dragRef.current = { id: v.id, mode, startX: e.clientX, origStart: v.start, origEnd: v.end };
    let moved = false;

    const snapTargets = [0, currentTime, totalDuration, ...vfx.filter((x) => x.id !== v.id).flatMap((x) => [x.start, x.end])];
    ...
    const up = () => {
      ...
      if (mode === "move" && !moved && onSeek) {
        if (currentTime > v.end) {
          onSeek(v.end);
          try { navigator.vibrate?.(10); } catch {}
        } else if (currentTime < v.start) {
          onSeek(v.start);
          try { navigator.vibrate?.(10); } catch {}
        }
      }
        ...
        <div className="absolute top-0 left-0 h-full" style={{
          ...
          {packed.map(({ item: v, lane }) => {
            ...
            return (
              <div 
                ...
                className={`absolute flex items-center justify-between cursor-grab shadow-md transition-all ${
                  focused && isSelected
                    ...
                    : "rounded-xl border border-amber-500/40 hover:border-amber-400/70 opacity-90 overflow-hidden"
                }`}
              ...
              >
                {/* Seamless unified selection outline spanning handles and VFX cleanly */}
                {focused && isSelected && (
                  <div className="absolute -left-3.5 sm:-left-4 -right-3.5 sm:-right-4 -top-[2px] -bottom-[2px] rounded-xl border-2 border-amber-400 ring-2 ring-amber-400/40 pointer-events-none z-30" />
                )}
                ...
                {focused && isSelected && (
                  <TimelineTrimHandle
                    side="left"
                    variant="amber"
                    onPointerDown={(e) => onDown(e, v, "left")}
                ...
                <span className="text-[8px] text-white/70 px-0.5 z-10">{Math.round(v.intensity * 100)}%</span>
                {focused && isSelected && (
                ...
                {focused && isSelected && (
                  <TimelineTrimHandle
                    side="right"
                    variant="amber"
                    onPointerDown={(e) => onDown(e, v, "right")}
...
VfxTimeline.displayName = "VfxTimeline";

export default VfxTimeline;
