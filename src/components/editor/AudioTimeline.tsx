 ...
 */
const AudioTimeline = memo(({ currentTime, pxPerSec, containerW, isPlaying, focused, onSeek, onAddClick, onFocus }: Props) => {
  const { audioTracks, totalDuration, updateAudioTrack, removeAudioTrack, audioBeats, selectedAudioTrackId, setSelectedAudioTrackId } = useMedia();
        ...
        >
          ...
          {audioTracks.map((a) => (
            <AudioBlock
              ...
              focused={focused}
              isSelected={selectedAudioTrackId === a.id}
              onSelect={() => setSelectedAudioTrackId(a.id)}
              pxPerSec={pxPerSec}
...
const AudioBlock = ({ track, pxPerSec, containerW, currentTime, totalDuration, top = 2, focused, isSelected, onSelect, onSeek, onMove, onResize, onFade, onMute, onRemove }: BlockProps) => {
  const { audioBeats = [], audioTracks = [], selectedAudioTrackId } = useMedia();
  ...
  // Determine beats belonging to this specific track
  const effectiveBeats: number[] = track.beats !== undefined
    ? (track.beats || [])
  ...
  const onPointerDown = (e: React.PointerEvent, kind: "move" | "in" | "out" | "fadeIn" | "fadeOut") => {
    ...
    const up = () => {
      ...
      if (kind === "move" && !moved && onSeek) {
        ...
        } else if (currentTime < track.start) {
          onSeek(track.start);
          try { navigator.vibrate?.(10); } catch {}
        }
      }
  ...
  return (
    ...
    >
      ...
      {effectiveBeats.map((b, i) => {
        if (b < track.start - 0.01 || b > track.start + track.duration + 0.01) return null;
        const pct = ((b - track.start) / Math.max(0.01, track.duration)) * 100;
        return (
          <div
            key={`beat-block-${i}`}
            className="absolute top-0 bottom-0 w-0.5 bg-cyan-300 shadow-[0_0_8px_rgba(6,182,212,0.95)] z-[8] pointer-events-none"
            style={{ left: `${Math.max(0, Math.min(100, pct))}%` }}
          >
            <div className="absolute top-0.5 left-1/2 -translate-x-1/2 w-2 h-2 rotate-45 bg-cyan-200 border border-white shadow flex items-center justify-center">
