...
import { X, Play, Pause, Repeat, Check, Eye, EyeOff, Megaphone, Flame, Wand2 } from "lucide-react";
...
const TRANSITION_PROMOS = [
  {
    id: "kinetic-pro",
    title: "مجموعة الانتقالات الحركية Kinetic Pro ⚡",
    titleEn: "Kinetic Pro Transitions Bundle",
    desc: "انتقالات بصرية فائقة النعومة مع اهتزازات وتأثيرات تمويه حركي مذهلة",
    descEn: "Ultra-smooth motion transitions with realistic camera shake & motion blur",
    color: "linear-gradient(135deg, #7c3aed 0%, #3b82f6 100%)",
    badge: "تحديث جديد",
    badgeEn: "NEW UPDATE",
    characterImg: "https://images.unsplash.com/photo-1550745165-9bc0b252726f?auto=format&fit=crop&w=150&q=70&fm=webp",
    transitionType: "ripple" as TransitionType,
  },
  {
    id: "retro-vhs",
    title: "انتقالات شريط الفيس بوك القديم VHS Retro 📺",
    titleEn: "Retro VHS Glitch Transitions",
    desc: "أضف طابع الثمانينات الكلاسيكي مع تأثيرات تداخل الإشارة وتشويش الشاشة",
    descEn: "Give your video an authentic 80s feel with signal glitches & tracking lines",
    color: "linear-gradient(135deg, #ec4899 0%, #f43f5e 100%)",
    badge: "حصري بريميوم",
    badgeEn: "PREMIUM VHS",
    characterImg: "https://images.unsplash.com/photo-1511512578047-dfb367046420?auto=format&fit=crop&w=150&q=70&fm=webp",
    transitionType: "glitch" as TransitionType,
  }
];
...
const TransitionPanel = ({ open, clipId, onClose }: Props) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setCurrentAdIndex((prev) => (prev + 1) % TRANSITION_PROMOS.length);
    }, 6000);
  ...
  const handleApplyPromoAd = (promo: typeof TRANSITION_PROMOS[0]) => {
    playSfx("success");
    apply(promo.transitionType);
    toast.success(en ? `Applied ${promo.titleEn}!` : `تم تطبيق الانتقال ${promo.title}!`);
  };
  ...
  const activePromo = TRANSITION_PROMOS[currentAdIndex];
