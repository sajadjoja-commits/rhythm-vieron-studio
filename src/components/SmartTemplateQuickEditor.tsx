...
export default function SmartTemplateQuickEditor({ initialTemplate, onBack, onOpenFullEditor }: Props) {
  ...
  const [allTracks, setAllTracks] = useState<BuiltinTrack[]>(() => [
    ...getSavedLibraryTracks(),
    ...BUILTIN_TRACKS,
  ]);
  const [selectedTrack, setSelectedTrack] = useState<BuiltinTrack | null>(
    () => allTracks[0] || null
  );
  ...
  const handleGenerateMontage = async () => {
    ...
    setAnalysisStatusText(isRTL() ? "بدء التحليل الذكي لحركة المشاهد..." : "Starting smart moment analysis...");
    ...
    try {
      ...
      const res = await runAutoMontage(itemsToProcess, selectedTemplate, [], musicUrl, {
        ...
        targetDuration,
        onProgress: (info) => {
          setAnalyzingProgress(info.percent);
          setAnalysisStatusText(isRTL() ? info.messageAr : info.messageEn);
        },
      });
